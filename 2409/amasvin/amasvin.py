class Drink:
    ...
    def set_cup(self):
        for index, cup in enumerate(Drink._CUPS):     #컵 종류 보여줌
            print(f'{index + 1}: {cup}')
    ...
    def set_ice(self):
        for index, ice in enumerate(Drink._ICES):  #얼음량 종류를 보여주자
            print(f'{index + 1}: {ice}')
    ...
    def set_sugar(self):
        for index, sugar in enumerate(Drink._SUGARS):           #당도 종류를 보여주자
            print(f'{index + 1}: {sugar}')
        sugar = input('당도를 선택하세요: ')    #당도 선택
        # if sugar == '':
        #     self.sugar = 2
        # else:
        #     self.sugar = int(sugar) -1  #self.sugar에 넣자
...
class Bubbletea(Drink):
    _PEARLS = ('타피오카', '코코', '알로에', '젤리')
    def __init__(self, name, price):
        super().__init__(name, price)# 부모 초기화 호출
        self.pearl = 0

    def set_pearl(self):
        for index, pearl in enumerate(Bubbletea._PEARLS):           #펼 종류 보여주자
        ...
        #     self.pearl = 0
        # else:
        #     self.pearl = int(pearl) -1  # self.pearl에 넣자

    def order(self):
        super().order()     # 부모클래스의 order()호출
        self.set_pearl()    # set_pearl() 호출
...
class Order:
    ...
    def sum_price(self):
        sum_value = 0
        for drink in self.order_menu:     #self.order_menu하나식 꺼내서 가격 합산
